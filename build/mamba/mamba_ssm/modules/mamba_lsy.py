...
class Mamba(nn.Module):
    def __init__(
        ...
        dt = torch.exp(torch.rand(self.d_inner, **factory_kwargs)*math.log(dt_max/dt_min) + math.log(dt_min)).clamp(min=dt_init_floor)
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        xz_f, xz_b = torch.chunk(xz, 2, dim=1)  # (B, D, L) -> (B, D/2, L), (B, D/2, L)
        xz_b = xz_b.flip([-1])
        xz = torch.cat([xz_f, xz_b], dim=0)  # (2B, D/2, L)
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            out = out.chunk(2)  # (2B, D/2, L) -> (B, D/2, L), (B, D/2, L)
            out = torch.cat([out[0], out[1].flip([-1])], dim=1)  # (B, D, L)
